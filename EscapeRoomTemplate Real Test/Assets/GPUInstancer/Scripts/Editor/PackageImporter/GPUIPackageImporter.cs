    ...
    public static class GPUIPackageImporter
    {
        ...
        public static bool ImportPackages(GPUIPackageImporterData packageImporterData)
        {
            ...
            for (int d = 0; d < packageImporterData.packageDefinitions.Length; d++)
            {
                ...
                if (packageImporterData.importedPackageInfos != null)
                {
                    if (!packageImporterData.forceReimport)
                    {
                        ImportedPackageInfo importedPackageInfo = packageImporterData.importedPackageInfos.Find(pi => pi.packageURL.Equals(packageURL));
                    ...
                    packageImporterData.importedPackageInfos = new List<ImportedPackageInfo>();
                ...
                if (isSuccessfull)
                {
                    ...
                    packageImporterData.importedPackageInfos.RemoveAll(ip => ip.packageURL.Equals(packageURL));
                    packageImporterData.importedPackageInfos.Add(new ImportedPackageInfo
                    {
